...
class BatchCollator:
    ...
    def __call__(self, batch):
        ...
        img_fliplr = default_collate(transposed_batch[1])
        img_flipud = default_collate(transposed_batch[2])
        img_fliplrud = default_collate(transposed_batch[3])
        img_ids = default_collate(transposed_batch[5])

        if self.is_train:
            list_targets = transposed_batch[4]
            targets = Container(
                {key: default_collate([d[key] for d in list_targets]) for key in list_targets[0]}
            )
        else:
            targets = None
        return images, img_fliplr, img_flipud, img_fliplrud, targets, img_ids
