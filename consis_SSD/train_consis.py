...
def train(cfg, args):
    ...
    teacher = build_detection_model(cfg)
    teacher.to(device)
    ...
    model = do_train(cfg, model, teacher, train_loader, optimizer, scheduler, checkpointer, device, arguments, args)
    return model


def main():
    parser = argparse.ArgumentParser(description='Single Shot MultiBox Detector Training With PyTorch')
    parser.add_argument('--consis_mode', type=str, default='lr', help='lr, ud, lrud, left and right or up and down')
    parser.add_argument(
        "--config-file",
    ...
    parser.add_argument('--eval_step', default=100, type=int, help='Evaluate dataset every eval_step, disabled when eval_step < 0, 默认初始值为2500')
