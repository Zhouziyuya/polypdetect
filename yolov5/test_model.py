# load "DetectionModel" to observe the output of the 3 detection heads
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, imgs_flip, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                pred_flip = model(imgs) # {[16,3,80,80,6], [16,3,40,40,6], [16,3,20,20,6]}
                ...
                loss = 0
                if confidence>0.5:
                    consis_conf_loss1 = (kldivloss(pred[0][:,:,:,:,4].log(), pred_flip_consis[0][:,:,:,:,4]).sum(-1).mean()+ \
                                            kldivloss(pred_flip_consis[0][:,:,:,:,4].log(), pred[0][:,:,:,:,4]).sum(-1).mean())/2
                    consis_conf_loss2 = (kldivloss(pred[1][:,:,:,:,4].log(), pred_flip_consis[1][:,:,:,:,4]).sum(-1).mean()+ \
                                            kldivloss(pred_flip_consis[1][:,:,:,:,4].log(), pred[1][:,:,:,:,4]).sum(-1).mean())/2
                    consis_conf_loss3 = (kldivloss(pred[2][:,:,:,:,4].log(), pred_flip_consis[2][:,:,:,:,4]).sum(-1).mean()+ \
                                            kldivloss(pred_flip_consis[2][:,:,:,:,4].log(), pred[2][:,:,:,:,4]).sum(-1).mean())/2
                    ...
                    loss = consis_conf_loss1+consis_conf_loss2+consis_conf_loss3+consis_x+consis_ywh
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='runs/train/noaug/weights/best.pt', help='initial weights path')
    ...
    parser.add_argument('--name', default='consis_conf0.5', help='save to project/name')
